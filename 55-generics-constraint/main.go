// ASSIGNMENT
//
// We have different kinds of "line items" that we charge our customer's credit
// cards for. Line items can be things like "subscriptions" or "one-time
// payments" for email usage.
//
// Complete the chargeForLineItem function. First, it should check if the user
// has a balance with enough funds to be able to pay for the cost of the
// newItem. If they don't, then return an "insufficient funds" error and zero
// values for the other return values.
//
// If they do have enough funds:
//
// - Add the line item to the user's history by appending the newItem to the
//   slice of oldItems. This new slice is your first return value.
// - Calculate the user's new balance by subtracting the cost of the new item
//   from their balance. This is your second return value.
